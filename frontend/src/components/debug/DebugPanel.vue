  ...
  >
    <a-spin :spinning="debug.running">
      <div v-if="debug.finalResult" style="margin-bottom: 12px">
        <a-tag :color="debug.finalResult.status === 'succeeded' ? 'success' : 'error'">
          {{ debug.finalResult.status }}
        </a-tag>
        <span class="af-mono af-muted">总耗时 {{ debug.finalResult.total_ms }}ms</span>
      ...
      <div v-else class="af-muted">点击「调试运行」后，每个节点的输入/输出/状态/耗时会显示在这里。</div>
    </a-spin>
  </a-drawer>
</template>
